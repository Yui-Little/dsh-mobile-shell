...
function apply(ctx) {
    ...
    ctx.effect(() => {
        ...
        let hideTimer = 0;
        let lastVerdict = '';
        const showPanel = (verdict) => {
            if (lastVerdict === verdict && panel !== null)
                return;
            lastVerdict = verdict;
            panel?.remove();
        ...
        const diagnose = () => {
            ...
            for (const textarea of composerTextareas()) {
                ...
                if (busy || textarea.disabled || textarea.readOnly || rect.height < 30 || !hitTextarea) {
                    const topDesc = top === null
                        ? 'null'
                        : `${top.tagName.toLowerCase()}.${String(top.className ?? '').split(' ').slice(0, 2).join('.')}`;
                    lines.push(`ta[${phase}] ${state} rect=${Math.round(rect.width)}x${Math.round(rect.height)}@y${Math.round(rect.top)} hit=${topDesc}${hitTextarea ? '✓' : '✗'}`);
                }
            }
            if (lines.length === 0)
                return;
