...
function apply(ctx) {
    ...
    ctx.effect(() => {
        ...
        input.sink = (session, text, imageIds, mode) => {
            const files = (0, attachmentStore_ts_1.pendingAttachmentsOf)(session.sessionId);
            if (files.length > 0) {
    ...
    ctx.effect(() => {
        ...
        const onKeyDown = (event) => {
            ...
            if (target === null || !(target instanceof HTMLTextAreaElement))
                return;
            if (target.closest('[data-mobile-nav-composer]') === null)
                return;
            event.preventDefault();
            event.stopPropagation();
    ...
    ctx.effect(() => {
        const onPointerDown = (event) => {
            if (!(event.target instanceof Element))
                return;
            const el = event.target.closest('[data-mobile-nav-composer] button[aria-haspopup="listbox"], [data-mobile-nav-composer] [data-mobile-nav="attach"]');
            if (el === null)
                return;
            event.preventDefault();
            event.stopPropagation();
    ...
    ctx.effect(() => {
        ...
        if (!narrow.matches)
            return () => { };
        const isComposerTextarea = (node) => node instanceof HTMLTextAreaElement && node.closest('[data-mobile-nav-composer]') !== null;
        const keyboardOpen = () => {
        ...
        const onPointerDown = (event) => {
            ...
            const textarea = target instanceof HTMLTextAreaElement
                ...
                : target instanceof Element
                    ...
                    : null;
            if (textarea === null || !isComposerTextarea(textarea))
                return;
