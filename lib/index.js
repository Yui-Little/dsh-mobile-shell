...
function inferThinkingFormat(modelId) {
    const id = modelId.toLowerCase();
    if (/(deepseek|glm|qwen|doubao|kimi|moonshot|minimax)/.test(id))
        return 'deepseek';
    return 'openai';
}
/**
        ...
        for (const model of models) {
            ...
            if (model.reasoningEfforts !== false && api === 'openai-completions') {
                ...
                if (!hasThinking || !hasSupports) {
                    ...
                    if (!hasThinking)
                        target.thinkingFormat = inferThinkingFormat(typeof model.id === 'string' ? model.id : '');
                    if (!hasSupports)
