 ...
 */
function tryService(read) {
    try {
        return read();
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (message.includes('inactive context'))
            return undefined;
        throw error;
    }
}
...
function captureServices(childCtx) {
    return {
 ...
 */
function registerExact(webServer, path, handler) {
    ...
    if (existing !== undefined) {
        ...
        return () => {
            ...
            if (current !== undefined && current.handler === handler)
                raw.exact?.delete(path);
        };
    }
...
async function handleMarketplaceTranslate(scoped, req, res) {
    ...
    const def = settings.get('agent-default-model');
